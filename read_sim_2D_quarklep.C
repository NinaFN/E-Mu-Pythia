...
void read_sim_2D_quarklep() {
    ...
    TH2F *qe2DPartPt = new TH2F("qe_part_pt","", 50, 5.0, 60.0, 50, 0.0, 40.0);
    TH2F *qe2DPt = new TH2F("qe_pt","p_{T}^{Q} vs p_{T}^{e};p_{T}^{Q};p_{T}^{e}", 50, 5.0, 60.0, 50, 0.0, 40.0);
    ...
    TH2F *qm2DPartPt = new TH2F("qm_part_pt","", 50, 5.0, 60.0, 50, 0.0, 40.0);
    TH2F *qm2DPt = new TH2F("qm_pt","p_{T}^{Q} vs p_{T}^{#mu};p_{T}^{Q};p_{T}^{#mu}", 50, 5.0, 60.0, 50, 0.0, 40.0);
    ...
    for(std::vector<double>::iterator it = binLuminocity->begin(); it != binLuminocity->end(); ++it){
        ...
        qe2DPartEta->Reset();
        qeTuples[binCount]->Draw("etaL:etaQ>>qe_part_eta","semilepFlag==1");
        qe2DPartEta->Scale(1/(*it),"width");
        qe2DEta->Add(qe2DPartEta);

        qe2DPartEtaCut->Reset();
        qeTuples[binCount]->Draw("etaQ:etaL>>qe_part_etacut","semilepFlag==1");
        qe2DPartEtaCut->Scale(1/(*it),"width");
        qe2DEtaCut->Add(qe2DPartEtaCut);

        qe2DPartPt->Reset();
        qeTuples[binCount]->Draw("ptL:ptHat>>qe_part_pt","semilepFlag==1");
        qe2DPartPt->Scale(1/(*it),"width");
        qe2DPt->Add(qe2DPartPt);

        qe2DPartPhi->Reset();
        qeTuples[binCount]->Draw("phiL:phiQ>>qe_part_phi","semilepFlag==1");
        qe2DPartPhi->Scale(1/(*it),"width");
        qe2DPhi->Add(qe2DPartPhi);

        qe2DPartTheta->Reset();
        qeTuples[binCount]->Draw("thetaL:thetaQ>>qe_part_theta","semilepFlag==1");
        qe2DPartTheta->Scale(1/(*it),"width");
        qe2DTheta->Add(qe2DPartTheta);

        qe2DPartThetaCalc->Reset();
        qeTuples[binCount]->Draw("thetaL+thetaQ:thetaQ>>qe_part_thetacalc","semilepFlag==1");
        qe2DPartTheta->Scale(1/(*it),"width");
        ...
        qm2DPartEta->Reset();
        qmTuples[binCount]->Draw("etaL:etaQ>>qm_part_eta","semilepFlag==1");
        qm2DPartEta->Scale(1/(*it),"width");
        qm2DEta->Add(qm2DPartEta);

        qm2DPartEtaCut->Reset();
        qmTuples[binCount]->Draw("etaQ:etaL>>qm_part_etacut","semilepFlag==1");
        qm2DPartEtaCut->Scale(1/(*it),"width");
        qm2DEtaCut->Add(qm2DPartEtaCut);

        qm2DPartPt->Reset();
        qmTuples[binCount]->Draw("ptL:ptHat>>qm_part_pt","semilepFlag==1");
        qm2DPartPt->Scale(1/(*it),"width");
        qm2DPt->Add(qm2DPartPt);

        qm2DPartPhi->Reset();
        qmTuples[binCount]->Draw("phiL:phiQ>>qm_part_phi","semilepFlag==1");
        qm2DPartPhi->Scale(1/(*it),"width");
        qm2DPhi->Add(qm2DPartPhi);

        qm2DPartTheta->Reset();
        qmTuples[binCount]->Draw("thetaL:thetaQ>>qm_part_theta","semilepFlag==1");
    ...
    ////Plotting
    // qes
    TFile *outf =  new TFile("sim_hists_2D_quarklep_semilep.root", "RECREATE");
